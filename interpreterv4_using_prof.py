...
class LazyValue:
    ...
    def evaluate(self):
        if not self.evaluated:
            # Temporarily push the environment to handle scope
            print(self.environment)
            self.interpreter.env.push_new_env(self.environment)
            self.result = self.interpreter.__eval_expr(self.expression)
            self.interpreter.env.pop_func()
            self.evaluated = True
        return self.result
...
class Interpreter(InterpreterBase):
    ...
    def __call_func_aux(self, func_name, actual_args):
        ...
        for formal_ast, actual_ast in zip(formal_args, actual_args):
            result = copy.copy(LazyValue(actual_ast, self, self.env.get_curr_env()))
    ...
    def __assign(self, assign_ast):
        ...
        if expression.elem_type in [
            ...
            InterpreterBase.VAR_NODE,  
            self.BIN_OPS,    
            Interpreter.NEG_NODE,      
            Interpreter.NOT_NODE,      
        ]:
            value = LazyValue(expression, self, self.env.get_curr_env())
        else:
            value = self.__eval_expr(expression)
